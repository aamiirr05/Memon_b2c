...
const EnquiryContact = () => {
  const { contactEnquiry } = useOutletContext();
  const { isUpdating, handleStatus, handleDelete } = useEnquiryStore();

  const totalEnquiry = contactEnquiry?.data?.data.length;

  const approvedEnquiry = contactEnquiry?.data?.data.filter((i) => {
    return i.status == 'Approved';
  });

  const pendingEnquiry = contactEnquiry?.data?.data.filter((i) => {
    return i.status == 'Pending';
  });
  const rejectedEnquiry = contactEnquiry?.data?.data.filter((i) => {
  ...
  const data = contactEnquiry.data?.data || [];
    ...
    <>
      ...
      >
        <div className="mt-5 relative grid grid-rows-2 gap-5">
          <div
            className=" grid gap-1 w-full bg-darkgreen shadow-lg rounded-lg row-span-full text-center text-peach text-md font-zodiak p-2 grid-cols-[repeat(21,minmax(70px,1fr))] lg:grid-cols-[repeat(18,minmax(0,1fr))]"
          ...
          >
            ...
            <div className="py-2 px-2 col-span-3">Message</div>
          ...
          <div className="rounded-xl flex flex-col gap-3 w-full  ">
            {contactEnquiry.data && !contactEnquiry?.data?.data.length == 0 ? (
              currentItems.map((e, i) => {
                return (
                  <div
                    key={i}
                    className="grid gap-1 hover:bg-darkgreen/5 shadow-lg rounded-md bg-peach/30 text-darkgreen text-center text-xs font-jakarta p-2 grid-cols-[repeat(21,minmax(70px,1fr))] lg:grid-cols-[repeat(18,minmax(0,1fr))]"
                    // style={{ gridTemplateColumns: 'repeat(21, minmax(0, 1fr))' }}
                  >
