...
const EnquiryCustom = () => {
  const { customEnquiry } = useOutletContext();
  const { isUpdating, handleStatus, handleDelete } = useEnquiryStore();

  const totalEnquiry = customEnquiry?.data?.data.length;

  const approvedEnquiry = customEnquiry?.data?.data.filter((i) => {
    return i.status == 'Approved';
  });

  const pendingEnquiry = customEnquiry?.data?.data.filter((i) => {
    return i.status == 'Pending';
  });
  const rejectedEnquiry = customEnquiry?.data?.data.filter((i) => {
  ...
  const data = customEnquiry.data?.data || [];
    ...
    <>
      ...
      >
        <div className="mt-5 relative grid grid-rows-2 gap-5">
          <div
            className=" grid gap-1 w-full bg-darkgreen shadow-lg rounded-lg row-span-full text-center text-peach text-md font-zodiak p-2 grid-cols-[repeat(21,minmax(70px,1fr))] lg:grid-cols-[repeat(25,minmax(0,1fr))]"
          ...
          >
            ...
            <div className="py-2 px-2 col-span-2">Additional Info</div>
            <div className="py-2 px-2 col-span-3">Total Person</div>
            <div className="py-2 px-2 col-span-2">Room Type</div>
            <div className="py-2 px-2 col-span-2 ">Mak Hotel</div>
            <div className="py-2 px-2 col-span-2 ">Med Hotel</div>
          ...
          <div className="rounded-xl flex flex-col gap-3 w-full  ">
            {customEnquiry.data && !customEnquiry?.data?.data.length == 0 ? (
              currentItems.map((e, i) => {
                return (
                  <div
                    key={i}
                    className="grid gap-1 hover:bg-darkgreen/5 shadow-lg rounded-md bg-peach/30 text-darkgreen text-center text-xs font-jakarta p-2 grid-cols-[repeat(21,minmax(70px,1fr))] lg:grid-cols-[repeat(25,minmax(0,1fr))]"
                    // style={{ gridTemplateColumns: 'repeat(21, minmax(0, 1fr))' }}
                  >
                    <div className="py-2 px-2 col-span-2">
                      <span>{e.custom_package_id}</span>
                    </div>
                    <div className="py-2 px-2 col-span-2">
                    ...
                    <div className="py-2 px-2 col-span-2">
                      <span>{e.additional_info}</span>
                    </div>
                    <div className="py-2 px-2 col-span-3 flex flex-col gap-1 justify-start items-center">
                      <>
                        <span> Adults({e.adults})</span>
                        <span>
                          Children(
                          {e.kids})
                        </span>
                    ...
                    <div className="py-2 px-2 col-span-2 ">
                      <span>{e.makkah_hotel_name}</span>
                    </div>
                    <div className="py-2 px-2 col-span-2 ">
                      <span>{e.medina_hotel_name}</span>
                    </div>
