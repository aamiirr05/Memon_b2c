...
const EnquiryUmrah = () => {
  const { umrahEnquiry } = useOutletContext();
  const { isUpdating, handleStatus, handleDelete } = useEnquiryStore();

  const totalEnquiry = umrahEnquiry?.data?.data.length;

  const approvedEnquiry = umrahEnquiry?.data?.data.filter((i) => {
    return i.status == 'Approved';
  });

  const pendingEnquiry = umrahEnquiry?.data?.data.filter((i) => {
    return i.status == 'Pending';
  });
  const rejectedEnquiry = umrahEnquiry?.data?.data.filter((i) => {
  ...
  const data = umrahEnquiry.data?.data || [];
      ...
      >
        <div className="mt-5 relative grid grid-rows-2 gap-5">
          ...
          <div className="rounded-xl flex flex-col gap-3 w-full  ">
            {umrahEnquiry.data && !umrahEnquiry?.data?.data.length == 0 ? (
