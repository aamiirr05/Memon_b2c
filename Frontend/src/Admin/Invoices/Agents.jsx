...
const emptyForm = {
  ...
  gst_number: '', bank_name: '', account_number: '',
...
const Agents = () => {
  ...
  const handleSubmit = async () => {
    ...
    let success;
    if (editingAgent) {
      success = await updateAgent(editingAgent.agent_id, form);
    } else {
      success = await createAgent(form);
    }
    if (success) {
      setForm(emptyForm);
      setShowForm(false);
  ...
  return (
    <div className="w-full">
      ...
      {/* Create / Edit Form */}
      {showForm && (
        <div className="bg-white border border-darkgreen/20 rounded-xl p-6 mb-8 shadow-sm">
          <h3 className="font-zodiak font-semibold text-lg mb-5">
            {editingAgent ? 'Edit Agent' : 'New Agent'}
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { label: 'Agent Name *', key: 'name', placeholder: 'e.g. Raza Tours' },
              ...
              { label: 'Bank Name', key: 'bank_name', placeholder: 'Bank of Baroda' },
              { label: 'Account Number', key: 'account_number', placeholder: 'Account number' },
              { label: 'IFSC Code', key: 'ifsc_code', placeholder: 'BARB0XXXXXX' },
              { label: 'Account Holder', key: 'account_holder', placeholder: 'Name on account' },
            ].map((field) => (
              <div key={field.key}>
            ...
            <div key={agent.agent_id} className="bg-white border border-darkgreen/15 rounded-xl p-5 shadow-sm hover:shadow-md transition-all">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleEdit(agent)} className="p-1.5 rounded-lg hover:bg-darkgreen/10 text-darkgreen transition-all">
                    <Edit2 size={15} />
                  </button>
                  <button onClick={() => handleDelete(agent.agent_id)} className="p-1.5 rounded-lg hover:bg-red-50 text-red-500 transition-all">
                    <Trash2 size={15} />
                  </button>
                </div>
              </div>
              <div className="space-y-1 text-sm font-jakarta text-darkgreen/60">
                ...
                {agent.bank_name && <p>🏦 {agent.bank_name}</p>}
                {agent.account_number && <p>💳 {agent.account_number}</p>}
              </div>
            </div>
          ))}
        </div>
